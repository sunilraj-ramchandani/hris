...
class pagibigController extends Controller
{
    ...
    public function add(Request $request){
        if(request('id')==''){
            ...
            $insert = DB::insert('insert into hris.pagibig (price_min,price_max,ee_rate,er_rate,created_by,created_at,company_id) values (?,?,?,?,?,?,?)',[request('from'),request('to'),request('ee_rate'),request('er_rate'),Session::get('user'),$created_date,Session::get('id')]); 
             ...
             if(request('from')==''){
                ...
                $update =DB::update('update hris.pagibig set price_min=?,price_max =?,ee_rate=?,er_rate=?,updated_at=?,updated_by=? where id = ?',[request('from'),request('to'),request('ee_rate'),request('er_rate') ,$updated_date,Session::get('user'),request('id')]);
