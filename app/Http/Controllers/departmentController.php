...
class departmentController extends Controller
{
    ...
    public function add(Request $request){
        if(request('id')==''){
            ...
            $insert = DB::insert('insert into hris.department (name,created_by,updated_by,company_id) values (?,?,?,?)',[request('name'),Session::get('user'),Session::get('user'),Session::get('id')]); 
             ...
             if(request('name')==''){
                ...
                $update =DB::update('update hris.department set name = ? ,updated_at=?,updated_by=? where id = ?',[request('name'), $updated_date,Session::get('user'),request('id')]);
